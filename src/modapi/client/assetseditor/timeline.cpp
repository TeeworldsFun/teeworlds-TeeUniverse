...
const char* CAssetsEditorGui_Timeline::s_CursorToolHints[] = {
...
CAssetsEditorGui_Timeline::CAssetsEditorGui_Timeline(CAssetsEditor* pAssetsEditor) :
...
CAssetsEditorGui_Timeline::~CAssetsEditorGui_Timeline()
{
...
void CAssetsEditorGui_Timeline::TimeScaleCallback(float Pos)
{
...
void CAssetsEditorGui_Timeline::ValueScrollCallback(float Pos)
{
...
void CAssetsEditorGui_Timeline::TimeScrollCallback(float Pos)
{
...
int CAssetsEditorGui_Timeline::TimeToTimeline(float Time)
{		
	return m_TimelineRect.x + m_TimeScale * (Time - m_TimeScroll);
}

float CAssetsEditorGui_Timeline::TimelineToTime(int X)
{
	return m_TimeScroll + (X - m_TimelineRect.x)/m_TimeScale;
}

void CAssetsEditorGui_Timeline::Update()
{
...
void CAssetsEditorGui_Timeline::Render()
{
	...
	if(m_CursorTool == CAssetsEditorGui_Timeline::CURSORTOOL_FRAME_ADD)
	{
	...
	else if(m_CursorTool == CAssetsEditorGui_Timeline::CURSORTOOL_FRAME_DELETE)
	{
		CAsset_SkeletonAnimation::CSubPath KeyFramePath = KeyFramePicking(m_CursorX, m_CursorY);
		if(!KeyFramePath.IsNull())
		{
	...
	else if(m_CursorTool == CAssetsEditorGui_Timeline::CURSORTOOL_FRAME_MOVE || m_CursorTool == CAssetsEditorGui_Timeline::CURSORTOOL_FRAME_COLOR)
	{
		CAsset_SkeletonAnimation::CSubPath KeyFramePath = KeyFramePicking(m_CursorX, m_CursorY);
		if(!KeyFramePath.IsNull())
		{
			if(
				m_CursorTool == CAssetsEditorGui_Timeline::CURSORTOOL_FRAME_MOVE ||
				(m_CursorTool == CAssetsEditorGui_Timeline::CURSORTOOL_FRAME_COLOR && KeyFramePath.GetType() == CAsset_SkeletonAnimation::CSubPath::TYPE_LAYERKEYFRAME)
			)
			{
				float Time = TimelineToTime(m_CursorX);
				int Frame = static_cast<int>(round(Time*static_cast<float>(TU_SKELETONANIMATION_TIMESTEP)));
...
CAsset_SkeletonAnimation::CSubPath CAssetsEditorGui_Timeline::KeyFramePicking(int X, int Y)
{
	...
	//Parent bones
	CAsset_Skeleton* pSkeletonParent = m_pAssetsEditor->AssetManager()->GetAsset<CAsset_Skeleton>(pSkeleton->m_ParentPath);
	if(pSkeletonParent)
	{
...
CAsset_Skeleton::CBonePath CAssetsEditorGui_Timeline::BonePicking(int X, int Y)
{
	if(!m_TimelineRect.IsInside(X, Y))
...
CAsset_Skeleton::CBonePath CAssetsEditorGui_Timeline::LayerPicking(int X, int Y)
{
	if(!m_TimelineRect.IsInside(X, Y))
		return CAsset_Skeleton::CBonePath::Null();
...
void CAssetsEditorGui_Timeline::OnButtonClick(int X, int Y, int Button)
{
	if(Button == KEY_MOUSE_1 && m_TimelineRect.IsInside(X, Y))
	{
		...
		else if(m_CursorTool == CAssetsEditorGui_Timeline::CURSORTOOL_FRAME_COLOR)
		{
			CAsset_SkeletonAnimation::CSubPath KeyFramePath = KeyFramePicking(X, Y);
			if(!KeyFramePath.IsNull() && KeyFramePath.GetType() == CAsset_SkeletonAnimation::CSubPath::TYPE_LAYERKEYFRAME)
			{
				m_pAssetsEditor->EditAssetSubItem(m_pAssetsEditor->m_EditedAssetPath, m_DragedElement.ConvertToInteger(), CAssetsEditorGui_Editor::TAB_SKELETONANIMATION_KEYFRAMES);
		...
		else if(m_CursorTool == CAssetsEditorGui_Timeline::CURSORTOOL_FRAME_MOVE)
		{
			CAsset_SkeletonAnimation::CSubPath KeyFramePath = KeyFramePicking(X, Y);
			if(!KeyFramePath.IsNull())
			{
...
void CAssetsEditorGui_Timeline::OnButtonRelease(int Button)
{
...
void CAssetsEditorGui_Timeline::OnMouseOver(int X, int Y, int RelX, int RelY, int KeyState)
{
	m_CursorX = X;
	m_CursorY = Y;
	...
	switch(m_Drag)
	{
		...
		case 2:
			if(m_CursorTool == CAssetsEditorGui_Timeline::CURSORTOOL_FRAME_MOVE)
			{
				float Time = TimelineToTime(X);
				int Frame = static_cast<int>(round(Time*static_cast<float>(TU_SKELETONANIMATION_TIMESTEP)));

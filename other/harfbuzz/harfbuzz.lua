HarfBuzz = {
	...
	OptFind = function (name, required)
		local check = function(option, settings)
			...
			if ExecuteSilent("pkg-config harfbuzz") == 0 then
				option.value = true
		...
		local apply = function(option, settings)
			if option.use_pkgconfig == true then
				settings.cc.flags:Add("`pkg-config --cflags harfbuzz harfbuzz-icu`")
				settings.link.flags:Add("`pkg-config --libs harfbuzz harfbuzz-icu`")
			elseif option.use_winlib > 0 then
				settings.cc.includes:Add(HarfBuzz.basepath .. "/include")
				if option.use_winlib == 32 then
					settings.link.libpath:Add(HarfBuzz.basepath .. "/lib/x86")
				else
					settings.link.libpath:Add(HarfBuzz.basepath .. "/lib/x64")
				end
